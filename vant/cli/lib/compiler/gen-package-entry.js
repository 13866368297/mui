...
function genComponentImports(components, options) {
  return components
    .map(name => {
      ...
      let path = common_1.getComponentAbsolutePath(name,options.build)
...
function genPluginImports(plugins,options) {
  if (plugins && plugins.length) {
    return plugins
      .map(name => {
        let path = common_1.getPluginAbsolutePath(name,options.build)
        if (options.pathResolver) {
          path = options.pathResolver(path);
        }
